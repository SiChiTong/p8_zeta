// zeta group project for Problem Set 8
// implements AMCL functionality for BABS control using odom and lidar

// general implementation adapted from lin_steering_wrt_odom
...
int main(int argc, char** argv) {
	ros::init(argc, argv, "zeta_amcl_control");
	ros::NodeHandle nh;

	SteeringController amcl(&nh);
	ros::Rate looprate(UPDATE_RATE);

	while(ros::ok()) {
		// activate amcl routines for robot control
		amcl.transform_odom_to_map();
		amcl.lin_steering_algorithm();

		// sleep before beginning next loop
		ros::spinOnce();
		looprate.sleep();
	}
	return 0;
